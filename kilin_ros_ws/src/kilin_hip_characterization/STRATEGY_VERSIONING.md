...
- Strategy `1.1.0` introduced raw-`motor_position` tracking. Strategy `2.0.0` retains that position reference: `position_diff` never changes a motor-position target. It is used only by the optional bounded angle-difference lift-assist FF policy.
...
2.0.0 loading policy. It ignores all `static_breakaway_*` values, normalizes
...
test holds/moves, never startup or recovery.
