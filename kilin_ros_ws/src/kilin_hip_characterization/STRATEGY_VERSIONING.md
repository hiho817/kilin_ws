...
- Strategy `1.1.0` introduced raw-`motor_position` tracking; the current `1.7.0` revision retains that reference and does not feed `position_diff` into the position target or FF direction. This isolates the worm-gear/PID experiment from angle-difference feedback chatter.
...
Strategy `1.7.0` has no fixed direct hip FF or directional gain parameters.
...
Strategy 1.7.0 retains explicit `filtered`/`raw` velocity-source selection with
